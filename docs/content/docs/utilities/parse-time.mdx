...
description: Parse Time Value — chronos-date API reference.
---
...
```typescript
import { parseMSec } from "chronos-date";
...
**Note**: This function is designed for parsing time into ms/sec, not absolute timestamps. For date/time parsing and manipulation, consider using [Chronos](/docs/api/chronos-class) class or other [date utilities](/docs/utilities/date).
