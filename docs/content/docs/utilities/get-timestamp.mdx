---
title: Get Timestamp
description: Get Timestamp — chronos-date API reference.
---
...
```typescript
import { getTimestamp } from "chronos-date";
...
- Exported as `getTimestamp` (lowercase `s`), separate from [`Chronos.getTimeStamp()`](/docs/api/get-set#gettimestamp) which returns timestamp in milliseconds.
