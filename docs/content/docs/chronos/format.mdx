...
`Chronos` supports a rich set of format tokens that you can use to customize how a date is displayed. These tokens work across [`format`](#format), [`formatUTC`](#formatutc), and [`formatStrict`](#formatstrict) methods as well as for other methods and utilities like [`formatDate`](/docs/utils/format-date#formatdate) that accept a format, and behave similarly to libraries like `Moment.js` or `Day.js`.
</Callout>
