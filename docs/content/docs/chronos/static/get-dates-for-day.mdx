...
description: Returns ISO date strings or Chronos instances for each occurrence of a weekday.
---
...
```ts pg tab="Chronos Instances"
import { Chronos } from 'chronos-date';

// Get Chronos instances for Fridays in Q3 2025 (rounded)
const chronosInstances = Chronos.getDatesForDay('Friday', {
  from: '2025-07-01',
  to: '2025-09-30',
  roundDate: true,
  format: 'chronos'
...
console.log(chronosInstances.map((c) => c.format()));
```
