...
You can inject plugins via the static [`Chronos.use()`](/docs/api/static-methods#use) or [`Chronos.register()`](/docs/api/static-methods#register) method or its wrapper function:
...
```ts
import { Chronos, INTERNALS } from "chronos-date/chronos";
import type { $Chronos } from "chronos-date/date/types";
...
// or import { Chronos } from "chronos-date/chronos";
