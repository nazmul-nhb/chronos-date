...
description: Format Date & Time — chronos-date API reference.
---
...
- [Chronos format methods](/docs/api/format) for similar date formatting capabilities within the Chronos class.
...
<Callout type="info">
`formatDate` supports a rich set of format tokens that you can use to customize how a date is displayed. These tokens work across [Chronos format](/docs/api/format) methods, and behave similarly to libraries like `Moment.js` or `Day.js`.
</Callout>
