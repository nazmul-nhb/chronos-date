...
description: Get Time Zone Details — chronos-date API reference.
---
...
```typescript
import { getTimeZoneDetails } from "chronos-date";
...
```typescript
import { getNativeTimeZoneId } from "chronos-date";
...
```typescript
import { getTimeZoneIds } from "chronos-date";
...
- Refer to [**Chronos**](/docs/api/names) class for methods like these.
