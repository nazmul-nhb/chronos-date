...
console.log(date.subtract(1, "month").format("YYYY-MM-DD"));

// Comparison
const other = new Chronos("2025-07-01");
console.log("Is before July?", date.isBefore(other, "month"));
console.log("Days between:", date.diff(other, "day"));
```
...
date.season(); // Get current season
```
