---
title: Plugins
description: Chronos Plugins — Chronos Documentation.
icon: Blocks
---

<Callout type="info" title="About Plugins">
...
- Plugins can be official (maintained by the core team) or custom (developed by users). [See the list of official plugins below](#-official-plugins) or [learn how to write your own plugin](#️-writing-a-custom-plugin).
...
You can inject plugins via the static [`Chronos.use()`](/docs/chronos/static-methods#use) or [`Chronos.register()`](/docs/chronos/static-methods#register) method or its wrapper function:
...
To enable a plugin, use the static [`Chronos.use()`](/docs/api/static-methods#use) or [`Chronos.register()`](/docs/api/static-methods#register) method **before creating instances**:
...
| :------------------- | :-----------------------------------------: | :--------------------------------------------- |
| `relativeTimePlugin` | <Copy message="Import Statement Copied!" afterCopy="Import Statement Copied!" text="import { relativeTimePlugin } from 'chronos-date/plugins/relativeTimePlugins';" /> | [compare](comparison#compare), [getRelativeYear](comparison#getrelativeyear), [getRelativeMonth](comparison#getrelativemonth), [getRelativeWeek](comparison#getrelativeweek), [getRelativeDay](comparison#getrelativeday), [getRelativeHour](comparison#getrelativehour), [getRelativeMinute](comparison#getrelativeminute), [getRelativeSecond](comparison#getrelativesecond), [getRelativeMilliSecond](comparison#getrelativemillisecond), [isToday](checkers#istoday), [isTomorrow](checkers#istomorrow), [isYesterday](checkers#isyesterday) |
| `timeZonePlugin`     | <Copy message="Import Statement Copied!" afterCopy="Import Statement Copied!" text="import { timeZonePlugin } from 'chronos-date/plugins/timeZonePlugin';" />         | [timeZone](conversion#timezone), [getTimeZoneName](names#gettimezonename), [getTimeZoneNameShort](names#gettimezonenameshort) |
| `durationPlugin`     | <Copy message="Import Statement Copied!" afterCopy="Import Statement Copied!" text="import { durationPlugin } from 'chronos-date/plugins/durationPlugin';" />         | [duration](calculation#duration), [durationString](calculation#durationstring) |
| `dateRangePlugin`     | <Copy message="Import Statement Copied!" afterCopy="Import Statement Copied!" text="import { dateRangePlugin } from 'chronos-date/plugins/dateRangePlugin';" />         | [getDatesInRange](calculation#getdatesinrange) |
| `fromNowPlugin`      | <Copy message="Import Statement Copied!" afterCopy="Import Statement Copied!" text="import { fromNowPlugin } from 'chronos-date/plugins/fromNowPlugin';" />           | [fromNow](format#fromnow) |
| `businessPlugin`     | <Copy message="Import Statement Copied!" afterCopy="Import Statement Copied!" text="import { businessPlugin } from 'chronos-date/plugins/businessPlugin';" />         | [isWeekend](checkers#isweekend), [isWorkday](checkers#isworkday), [isBusinessHour](checkers#isbusinesshour), [nextWorkday](conversion#nextworkday), [nextWeekend](conversion#nextweekend), [previousWorkday](conversion#previousworkday), [previousWeekend](conversion#previousweekend), [workdaysBetween](get-set#workdaysbetween), [workdaysInMonth](get-set#workdaysinmonth), [workdaysInYear](get-set#workdaysinyear), [weekendsBetween](get-set#weekendsbetween), [weekendsInMonth](get-set#weekendsinmonth), [weekendsInYear](get-set#weekendsinyear), [toAcademicYear](extras#toacademicyear), [toFiscalQuarter](extras#tofiscalquarter) |
| `seasonPlugin`       | <Copy message="Import Statement Copied!" afterCopy="Import Statement Copied!" text="import { seasonPlugin } from 'chronos-date/plugins/seasonPlugin';" />             | [season](names#season), [getSeasonName](names#season) |
| `dayPartPlugin`      | <Copy message="Import Statement Copied!" afterCopy="Import Statement Copied!" text="import { dayPartPlugin } from 'chronos-date/plugins/dayPartPlugin';" />           | [getPartOfDay](names#getpartofday) |
| `zodiacPlugin`       | <Copy message="Import Statement Copied!" afterCopy="Import Statement Copied!" text="import { zodiacPlugin } from 'chronos-date/plugins/zodiacPlugin';" />         | [getZodiacSign](names#getzodiacsign), [zodiac](names#getzodiacsign), [getZodiacMeta](names#getzodiacmeta) |
| `palindromePlugin`   | <Copy message="Import Statement Copied!" afterCopy="Import Statement Copied!" text="import { palindromePlugin } from 'chronos-date/plugins/palindromePlugin';" />     | [isPalindromeDate](checkers#ispalindromedate) |
| `greetingPlugin`   | <Copy message="Import Statement Copied!" afterCopy="Import Statement Copied!" text="import { greetingPlugin  } from 'chronos-date/plugins/greetingPlugin';" />     | [getGreeting](format#getgreeting) |
| `roundPlugin`   | <Copy message="Import Statement Copied!" afterCopy="Import Statement Copied!" text="import { roundPlugin  } from 'chronos-date/plugins/roundPlugin';" />     | [round](calculation#round) |
| `banglaPlugin`   | <Copy message="Import Statement Copied!" afterCopy="Import Statement Copied!" text="import { banglaPlugin } from 'chronos-date/plugins/banglaPlugin';" />     | [Bangla calendar methods](./docs/plugins/bangla-plugin#-methods-added-by-this-plugin) |
...
```ts
import { Chronos, INTERNALS } from "chronos-date";
import type { ChronosPlugin } from "chronos-date/types";
...
export const customPlugin: ChronosPlugin = ($Chronos) => {
  $Chronos.prototype.customMethod = function (user) {
...
```ts
import { Chronos, INTERNALS } from "chronos-date";
import type { ChronosPlugin, TimeZone, UTCOffset } from "chronos-date/types";
// Other imports
...
1. Always augment `Chronos` via module augmentation to ensure TypeScript recognizes your new methods using this subpath: `'chronos-date'`
2. Import `Chronos` and `INTERNALS` from `'chronos-date'` in your plugin file
