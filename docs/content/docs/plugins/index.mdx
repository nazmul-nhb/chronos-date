---
title: Plugins
description: Extend Chronos with built-in or custom plugins.
icon: Blocks
---
...
You can inject plugins via the static [`Chronos.use()`](/docs/chronos/static/use) or [`Chronos.register()`](/docs/chronos/static/register) method or its wrapper function.
...
To enable a plugin, use the static [`Chronos.use()`](/docs/chronos/static/use) or [`Chronos.register()`](/docs/chronos/static/register) method **before creating instances**:
...
- [`banglaPlugin`](/docs/plugins/bangla-plugin) - Integrate Bangla calendar system
- [`timezonePlugin`](/docs/plugins/timezone-plugin) - Handle time zone conversions and display time zone names
- [`relativeTimePlugin`](/docs/plugins/relative-time-plugin) - Handle relative time calculations and comparisons
- [`businessPlugin`](/docs/plugins/business-plugin) - Handle business hours and workdays
- [`durationPlugin`](/docs/plugins/duration-plugin) - Handle duration calculations and formatting
- [`dateRangePlugin`](/docs/plugins/date-range-plugin) - Handle date range calculations
- [`fromNowPlugin`](/docs/plugins/from-now-plugin) - Handle relative time calculations
- [`seasonPlugin`](/docs/plugins/season-plugin) - Handle season calculations
- [`dayPartPlugin`](/docs/plugins/day-part-plugin) - Handle day part calculations
- [`zodiacPlugin`](/docs/plugins/zodiac-plugin) - Handle zodiac calculations
- [`greetingPlugin`](/docs/plugins/greeting-plugin) - Handle greeting calculations
- [`roundPlugin`](/docs/plugins/round-plugin) - Handle round calculations
- [`palindromePlugin`](/docs/plugins/palindrome-plugin) - Handle palindrome date calculations
...
<Steps>
  <Step>
    <Callout type="idea" title="Create a Plugin">
    ```ts
    import { INTERNALS } from "chronos-date";
  ...
  <Step>
    <Callout type="success" title="Consume the Plugin">
...
<Callout type="idea" title="Tips">
 You can consume the custom plugin in the same file or another file where you want to use your custom method.
</Callout>
...
// The plugin function must be in a `.ts` or `.js` file
/** * Plugin to inject `customMethod` into Chronos instances. */
const customPlugin: ChronosPlugin = ($Chronos) => {
  $Chronos.prototype.customMethod = function (user) {
    // Example of accessing internals through protected static interface
    const internalDate = $Chronos[INTERNALS].internalDate(this);
    return `Hello ${user}, Welcome to custom plugin! Current date: { local: ${this} } { utc: ${internalDate.toISOString()} }`;
...
1. Always augment `Chronos` via module augmentation to ensure TypeScript recognizes new methods using the main module path: `'chronos-date'`
