...
description: Date Range Plugin — Chronos Plugin Documentation.
---
...
// Get array of Chronos instances
const chronosInRange = c.getDatesInRange({
    from: '2025-01-01',
    to: '2025-01-31',
    format: "chronos",
    skipDays: ['Saturday', 'Sunday'] // or [6, 0]
...
console.log(chronosInRange.map((c) => c.format()));
```
...
| ------------------------------------- | -------------------------------------------------------------------------- | ----------------------- |
| [`getDatesInRange`](#getdatesinrange) | Generate an array of ISO date strings or `Chronos` instances in the range. | `(string \| Chronos)[]` |
...
getDatesInRange<F extends ISODateFormat | 'chronos' = 'local'>(options?: RelativeDateRange<F>): DateRangeResult<F>
```
...
interface RelativeDateRange {
  /** Number of time units to move forward from current `Chronos` instance. Default: 4 */
  span?: number;
...
<Callout type="info" title="Similar Utility Function">
- [getDatesInRange](/docs/utils/get-dates-in-range)
</Callout>
